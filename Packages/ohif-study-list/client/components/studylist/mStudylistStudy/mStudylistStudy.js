...
Template.mStudylistStudy.helpers({
    checkMobileDevice () {
        //if($(window).width() < 800) {
        if($(window).width() < 961) {            
            return true;
...
Template.mStudylistStudy.onRendered(() => {
    const instance = Template.instance();
    const data = instance.data;
    let $row;    
    $row = instance.$('table.studylistStudy').first();
