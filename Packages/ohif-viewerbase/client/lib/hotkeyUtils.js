...
Meteor.startup(function() {
	...
	function updateViewports(viewports, status) {
        for (let vp = 0; vp < viewports.length; vp++) {
            if (status === true) {
                cornerstoneTools.referenceLines.tool.disable(viewports[vp]);
            } else {
                cornerstoneTools.referenceLines.tool.enable(viewports[vp]);
            }
        }
    }
    ...
    OHIF.commands.set(contextName, {
		...
		toggleMetadataDialog: {
          name: 'Show/Hide Metadata Dialog',
          action: viewportUtils.toggleMetadataDialog,
          disabled: () => !viewportUtils.isDownloadEnabled()
        },
		toggleReferenceLines: {
            name: 'Show/Hide Reference Lines',
            action () {
				let viewports = document.getElementsByClassName("imageViewerViewport");
				console.log('Reference Lines Clicked : ', OHIF.viewer.refLinesEnabled);
				console.log('Reference Lines Clicked Viewport : ', viewports);
				if (OHIF.viewer.refLinesEnabled) {
					updateViewports(viewports, true)
					setOHIFRefLines(false);
				} else {
					updateViewports(viewports, false)
					setOHIFRefLines(true);
				}
			}
        },
        toggleReportDialog: {
